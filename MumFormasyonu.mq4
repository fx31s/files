...
double iaos=iAO(ChartSymbol(ChartID()),ChartPeriod(ChartID()),i+5);
 ...
 if ( OrdersTotal() == 0 ) int ticket=OrderSend(Symbol(),OP_BUY,Lot,Ask,0,sl,tp,"",magic,0,clrNONE);
...
double iaos=iAO(ChartSymbol(ChartID()),ChartPeriod(ChartID()),i+5);
 ...
 if ( OrdersTotal() == 0 ) int ticket=OrderSend(Symbol(),OP_SELL,Lot,Bid,0,sl,tp,"",magic,0,clrNONE);
