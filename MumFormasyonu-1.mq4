...
//--- create timer
   ...
   //&& Close[i+2] > Open[i+2] && Close[i+3] > Open[i+3] && 
   && High[i+1] > High[i+2] && High[i+1] > High[i+3] && Close[i] < Open[i+1] ) {
   
   ObjectCreate(ChartID(),"RECUP"+i,OBJ_RECTANGLE,0,Time[i+1],High[i+1],Time[i+3],Open[i+1]);
   ObjectSetInteger(ChartID(),"RECUP"+i,OBJPROP_COLOR,clrCrimson);
   
   }
   
//Close[i+2] < Open[i+2] && Close[i+3] > Open[i+3] &&
   if ( Close[i+1] > Open[i+1] &&  Low[i+1] < Low[i+2] && Low[i+1] < Low[i+3] && Close[i] > Open[i+1] ) {
   
   ObjectCreate(ChartID(),"RECUPs"+i,OBJ_RECTANGLE,0,Time[i+1],Low[i+1],Time[i+3],Open[i+1]);
   ObjectSetInteger(ChartID(),"RECUPs"+i,OBJPROP_COLOR,clrChartreuse);
   }
   ...
   ObjectCreate(ChartID(),"RECUPs"+i,OBJ_RECTANGLE,0,Time[i],Low[i+1],Time[i+3],Open[i]);
   ObjectSetInteger(ChartID(),"RECUPs"+i,OBJPROP_COLOR,clrChartreuse);
   }
   ...
   ObjectCreate(ChartID(),"RECUPs"+i,OBJ_RECTANGLE,0,Time[i],High[i+1],Time[i+3],Open[i]);
