...
/*
     ...
     if ( (MarketInfo(sym,MODE_BID) - ma)/MarketInfo(sym,MODE_POINT) <= 500 && MarketInfo(sym,MODE_BID) > ma ) Print(sym,":",kalan,"Time:",TFtoStr(per));
...
double ma=iMA(sym, per, MC_W, ma_shift, MaMethod, MaPrice, i);
...
double mab=iMA(sym, per, MC_W, ma_shift, MaMethod, MaPrice, b);
...
/*
     ...
     ObjectSetInteger(ChartID(),"MCL"+i,OBJPROP_COLOR,clrDarkGray);
     ObjectSetInteger(ChartID(),"MCL"+i,OBJPROP_WIDTH,3);
...
/*
     ...
     ObjectCreate(ChartID(),"MCL"+i,OBJ_TREND,0,Time[i],ma,Time[i]+1000*PeriodSeconds(),ma);
     ObjectSetInteger(ChartID(),"MCL"+i,OBJPROP_RAY,False);
     ObjectSetInteger(ChartID(),"MCL"+i,OBJPROP_COLOR,clrBlack);
